app.controller('AdminController', function($scope,$http){
  ...
  app.controller('BookController', function(dataFactory,$scope,$http){
    $scope.data = [];
    $scope.libraryTemp = {};
    ...
    $scope.remove = function(item,index){
      var result = confirm("Are you sure delete this item?");
         if (result) {
        dataFactory.httpRequest('books/'+item.id,'DELETE').then(function(data) {
            $scope.data.splice(index,1);
        });
      }
    }
    $scope.refresh = function(item,index){
      dataFactory.httpRequest('/books').then(function(data) {
        $scope.data = data.data;
        $scope.totalItems = data.total;
      });
    }
